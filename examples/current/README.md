...
- `accepted_bootstrap_match.nl` shows the accepted ADR-0033 enum and `when` surface with the ADR-0034 typed parameter subject. The compiler records typed body parameters and indexes them as immutable body bindings; enum subject resolution, variant resolution, and exhaustiveness checking remain pending M0021 work. Payloads, destructuring, and implicit smart casts remain deferred.
