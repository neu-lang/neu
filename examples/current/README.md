...
- `accepted_bootstrap_match.nl` shows the accepted ADR-0033 enum and `when` surface with the ADR-0034 typed parameter subject. The compiler validates resolved enum parameter subjects, resolves qualified variant arms, diagnoses duplicate enum and match-arm cases, and requires either every declared variant or a wildcard. Payloads, destructuring, and implicit smart casts remain deferred.
- `accepted_bootstrap_ownership.nl` shows the accepted ADR-0035 surface for copyable `Int` locals and move-only `String` local transfers.
- `use_after_move_error.nl` demonstrates a later bare-name use of a moved `String` local, which reports `use_after_move` with the move origin recorded.
- `duplicate_match_arm_error.nl` demonstrates repeated qualified and wildcard match patterns. Their second occurrences are the primary locations for duplicate-match diagnostics.
- `non_exhaustive_match_error.nl` demonstrates an otherwise valid `when` that omits a declared variant and reports `non_exhaustive_match` on the subject.
- `parsed_bootstrap_enum.nl` is accepted by the current parser and records ordered no-payload enum variants. `when` parsing, qualified-arm resolution, and finite exhaustiveness checking are available.
