...
What smallest synchronous borrow and lifetime subset can the compiler implement without
...
Design 2 is ergonomic but too implicit for a first soundness implementation phase; it risks
treating ordinary reads or assignments as borrows without accepted rules.

Design 3 lets the compiler validate the core shared-or-exclusive invariant over
...
Choose design 3 for this implementation.

The compiler introduces a bootstrap borrow-analysis side-table model. It does not add
...
The region node is the lexical scope in which the borrow is live. For the compiler,
...
The compiler only checks equality of region identity for overlap. Nested, sibling,
...
For the compiler, a lifetime escape is diagnosed when a borrowed local is used as a
...
The compiler can implement borrow conflict and lifetime escape checking over explicit
...
ADR-0026, ADR-0035, and the compiler.
