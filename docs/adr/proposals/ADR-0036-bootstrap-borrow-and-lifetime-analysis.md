...
an accepted ADR, and not a valid basis for implementation until accepted by the
main-task Chief Architect decision.
...
Unsupported source-level borrow syntax or reference types must remain parser or
...
This resolves `docs/ambiguities/M0023-borrow-lifetime-semantics.md` only if
accepted. It does not define source-level borrow syntax or reference types.
