...
The compiler can validate the model without defining coroutine source syntax, runtime scheduling, detached
...
3. Use metadata-only task-scope and suspension records for this implementation.
4. Defer the compiler entirely until async syntax and runtime scheduling exist.

## Trade-offs

Concrete source syntax would exercise the eventual user-facing model, but
ADR-0024, ADR-0037, and prior implementation phase scoping deliberately keep unsupported
...
the priority order: sound static semantics must precede execution support.

Metadata-only records let the compiler validate the safety rules selected by ADR-0008
...
suspension borrowing without incremental validation after this implementation.

## Recommended Choice

Use a metadata-only bootstrap coroutine scope and suspension model.

The compiler introduces no source-level coroutine, `async`, `await`, task-spawn,
...
Input records are compiler side-table facts supplied by earlier or test
...
A child task is valid in this implementation only when its completion-or-cancellation scope is
...
The compiler reports `task_scope_escape` when a child task is not proven to complete
...
A borrow crossing a suspension point is valid in this implementation only when both of these
conditions hold:
...
The compiler reports `borrow_across_suspension` when a suspended-borrow record fails
...
Cancellation resource-safety in this implementation is limited to the static structured-scope
...
The compiler thread capability facts may be consumed by later task-boundary records,
but the compiler does not infer task captures or add new `Send` or `Share` rules.
...
- The compiler can implement structured-scope and suspension-borrow checkers without
...
- Runtime scheduling, cancellation propagation, and async frame lowering remain
  later future work.
- The compiler and later diagnostics can rely on stable diagnostic identifiers
