...
require_text "$ambiguity" 'No parser implementation may accept concrete expression, statement, or pattern syntax'
...
require_text "$ledger" '\| Coroutine syntax \| ambiguous \| ADR-0008, ADR-0009'
require_text "$ledger" '\| Unsafe block syntax \| ambiguous \| ADR-0018'
require_text "$ledger" 'M0013 expression, statement, and pattern parser is blocked on expression, statement, and pattern syntax authority'
...
echo "m0013-blocked: expression statement and pattern parser correctly blocked by syntax ambiguity"
