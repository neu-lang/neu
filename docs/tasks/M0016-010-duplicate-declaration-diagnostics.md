...
- `build_declaration_index` from `crates/compiler/src/name_resolution.rs`.
- Parser declaration metadata from `crates/compiler/src/parser.rs`.
...
2026-07-10 main_task=main-task test work phase=verify-tests-fail result=pass notes=cargo test -p compiler --test name_resolution failed before implementation because DeclarationIndexBuild did not expose diagnostics.
