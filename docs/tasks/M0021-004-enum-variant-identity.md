...
- `crates/compiler/src/parser.rs`: `ParsedEnumVariant`.
- `crates/compiler/src/module.rs`: module/package metadata conventions.
- `crates/compiler/src/name_resolution.rs`: stable source-ordered index patterns.
- Validation: `cargo test -p compiler --test name_resolution m0021_enum_variant_identity`;
...
- 2026-07-10 agent=Main phase=test-first result=fail evidence=`cargo test -p compiler --test name_resolution m0021_enum_variant_identity` failed because `build_enum_variant_index` was absent. handoff=Implementer
