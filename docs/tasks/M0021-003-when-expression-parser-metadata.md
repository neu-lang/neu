...
- `crates/newlang/src/parser.rs`: expression parsing and pattern parsing.
- `crates/newlang/src/ast.rs`: expression and pattern node conventions.
- Validation: `cargo test -p newlang --test parser m0021_when_expression`;
...
- 2026-07-10 agent=Main phase=test-first result=fail evidence=`cargo test -p newlang --test parser m0021_when_expression` failed because when-expression and match-arm metadata APIs were absent. handoff=Implementer
