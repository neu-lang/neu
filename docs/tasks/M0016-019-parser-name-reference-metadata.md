...
- Parser `NameExpression` support from `crates/newlang/src/parser.rs`
...
```text
2026-07-10 agent=Task-Decomposer phase=create-task result=pass notes=Created M0016 parser name-reference metadata task.
2026-07-10 agent=Test-Engineer phase=generate-tests result=pass notes=Updated parser tests and M0016 data-model validator before adding ParsedNameReference.
2026-07-10 agent=Test-Engineer phase=verify-tests-fail result=pass notes=cargo test -p newlang --test parser failed before implementation because ParseOutput.name_references was missing.
2026-07-10 agent=Implementer phase=implementation result=pass notes=Added ParsedNameReference metadata for simple identifier NameExpression nodes.
...
2026-07-10 agent=Adversarial-Engineer phase=adversarial-tests result=pass notes=docs/scripts/adversarial-check.sh created a soundness report after ordinary tests were recorded; concrete adversarial review found no lookup or excluded reference metadata.
2026-07-10 agent=Reviewer phase=review result=pass notes=docs/tasks/reviews/M0016-019-review.md approved parser name-reference metadata pending final CI gate.
