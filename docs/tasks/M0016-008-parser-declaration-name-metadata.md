...
```text
2026-07-10 agent=Task-Decomposer phase=create-task result=pass notes=Created M0016 parser declaration name metadata task.
2026-07-10 agent=Test-Engineer phase=generate-tests result=pass notes=Updated parser and docs validators before adding declaration name metadata.
2026-07-10 agent=Test-Engineer phase=verify-tests-fail result=pass notes=cargo test -p newlang --test parser failed before implementation because ParseOutput did not expose declaration_names.
2026-07-10 agent=Implementer phase=implementation result=pass notes=Added ParsedDeclarationName metadata for top-level function and type declarations without declaration-index population or lookup.
...
2026-07-10 agent=Adversarial-Engineer phase=adversarial-tests result=pass notes=docs/scripts/adversarial-check.sh created docs/tasks/soundness/M0016-008-soundness.md after ordinary tests were recorded.
2026-07-10 agent=Reviewer phase=review result=pass notes=docs/tasks/reviews/M0016-008-review.md approved parser-metadata-only scope pending final CI gate.
