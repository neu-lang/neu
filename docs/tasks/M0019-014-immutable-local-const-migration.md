...
- Generate tests: `cargo test -p compiler --test lexer --test parser --test name_resolution --test type_check`
- Verify tests fail: `cargo test -p compiler --test lexer --test parser --test name_resolution --test type_check`
...
2026-07-10 main_task=main-task test work phase=pre-implementation-validation result=expected-fail command="cargo test -p compiler --test lexer --test parser --test name_resolution --test type_check" evidence="33 E0599 errors: TokenKind::KwConst is absent (lexer.rs:56, :89, :93) and LocalBindingKind::Immutable is absent (parser.rs:404 and semantic tests); no assertion or fixture parse failure occurred."
2026-07-10 main_task=main-task test work phase=pre-implementation-validation result=expected-fail command="sh docs/tests/m0019-immutable-local-const-migration.sh" evidence="missing expected pattern in crates/compiler/src/lexer.rs: KwConst; confirms lexer/parser migration has not started."
