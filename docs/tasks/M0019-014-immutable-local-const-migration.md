...
- Status: `complete`
- Owner Agent: `Test Engineer` then `Implementer`
...
- Agent Prompts: `.codex/agents/task-decomposer.toml`, `.codex/agents/test-engineer.toml`, `.codex/agents/implementer.toml`, `.codex/agents/adversarial-engineer.toml`, `.codex/agents/diagnostics-engineer.toml`, `.codex/agents/reviewer.toml`, `.codex/agents/spec-compliance-auditor.toml`, `.codex/agents/build-engineer.toml`
...
- Test files to create or update before implementation: existing lexer, parser, name-resolution, type-check, flow, and ownership-facing Rust fixture files identified by the Test Engineer, plus the relevant task/docs validator.
...
- Diagnostics Engineer: verify ordinary recovery, spans, wording, and absence of a legacy-`val` diagnostic.
- Spec Compliance Auditor: verify every phase against ADR-0029 and the cross-phase invariant.
- Adversarial Engineer: verify no semantic conflation or forged metadata path.
- Reviewer and Simplicity Guardian: verify scope and lack of compatibility abstractions.
- Build Engineer: verify formatting, lint, workspace tests, and relevant frontend gates.
...
- Test files: existing lexer/parser/name-resolution/type-check/flow/ownership fixture files and their task validator, as identified by Test Engineer.
...
```text
2026-07-10 agent=Task-Decomposer/Roadmap-Planner phase=revalidation result=pass notes=ADR-0029 authority gate confirmed satisfied; M0019-014 revalidated and entered the tests-first workflow.
