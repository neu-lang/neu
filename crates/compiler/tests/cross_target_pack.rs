...
#[test]
fn links_overloaded_functions_through_the_x86_target_pack_without_execution() {
    let target = "x86_64-unknown-linux-gnu".parse::<Triple>().unwrap();
    let root = std::env::temp_dir().join(format!("neu-cross-overload-{}", std::process::id()));
    let _ = fs::remove_dir_all(&root);
    fs::create_dir_all(&root).unwrap();
    let output = root.join("program");
    let executable = compile_source_to_executable(
        "func choose(value: Int): Int { return 1; } func choose(value: Bool): Int { return 2; } public func main(): Int { return choose(true); }",
        SourceDriverOptions::new(
